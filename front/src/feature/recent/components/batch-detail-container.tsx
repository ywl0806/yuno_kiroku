...
export const BatchDetailContainer: FC<Props> = ({ batchId }) => {
    ...
    const dateLabel = useMemo(() => {
        if (!data?.pages[0]?.items[0]?.created_at) return ''
        const date = new Date(data.pages[0].items[0].created_at)
    ...
    return (
        <div className="flex h-full flex-col">
            <div className="flex items-center gap-3 border-b px-4 py-3">
                ...
                <span className="font-medium text-[1rem] text-accent-foreground">{dateLabel}</span>
            </div>
