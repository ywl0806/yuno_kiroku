...
import { useGetKidFaceImgs } from '@/feature/home/hooks/use-get-kid-face-imgs'
import { useNavigate } from 'react-router-dom'
import { getAgeLabel, getAgoLabel } from '@/feature/home/utils/age-label'
...
export const KidCircleRow: FC<Props> = ({ year, month }) => {
  ...
  return (
    <div className="flex gap-4 overflow-x-auto px-4 pb-1 justify-between w-full items-baseline">
      <div className="flex gap-4">
        {kidFaceImgs.kids.map((kid) => (
          <button
            key={kid.kid_id}
            type="button"
            onClick={() => {
      ...
      <div className="flex gap-4">
        {kidFaceImgs.fast_kids.map((kid) => (
          <button
            ...
            className="flex shrink-0 flex-col items-center"
          >
            <div className="size-20 overflow-hidden rounded-full border-2 border-white/80 shadow-md">
              <img src={kid.face_img_url} alt={kid.name} className="h-full w-full object-cover" />
            </div>
            <span className="text-xs font-semibold text-white drop-shadow">{kid.name}</span>
            <span className="text-xs text-white/80 drop-shadow">{getAgoLabel(new Date(year, month), new Date(kid.taken_at_year, kid.taken_at_month), t)}</span>
          </button>
        ))}
      </div>
    </div>
