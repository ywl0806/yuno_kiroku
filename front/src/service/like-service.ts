...
export const likeMediaItem = async (mediaItemId: number): Promise<void> => {
  await MyAxiosWithAuth.post(API_ROUTES.MEDIA_ITEM.LIKE(mediaItemId))
}

export const unlikeMediaItem = async (mediaItemId: number): Promise<void> => {
  await MyAxiosWithAuth.delete(API_ROUTES.MEDIA_ITEM.LIKE(mediaItemId))
}

export const isMediaItemLiked = async (mediaItemId: number): Promise<boolean> => {
