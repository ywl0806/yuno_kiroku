...
export const addTagToMediaItem = async (mediaItemId: number, tagId: number): Promise<void> => {
  await MyAxiosWithAuth.post(API_ROUTES.MEDIA_ITEM.TAGS(mediaItemId), { tag_id: tagId })
}

export const removeTagFromMediaItem = async (mediaItemId: number, tagId: number): Promise<void> => {
  await MyAxiosWithAuth.delete(API_ROUTES.MEDIA_ITEM.TAG_REMOVE(mediaItemId, tagId))
}

export const getTagsForMediaItem = async (mediaItemId: number): Promise<Tag[]> => {
