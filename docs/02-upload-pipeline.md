...
| 파일                                                                                                | 변경 내용                                    |
| --------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| [server/pkg/storage/storage_service.go](../server/pkg/storage/storage_service.go)                   | `GeneratePresignedPutURL()` 인터페이스 추가  |
| [server/pkg/storage/s3_storage_service.go](../server/pkg/storage/s3_storage_service.go)             | Presigned URL 구현                           |
| [server/internal/services/media_item_service.go](../server/internal/services/media_item_service.go) | `UploadImage()` → `GetPresignedURL()` 재설계 |
| [server/internal/handlers/media_item_handler.go](../server/internal/handlers/media_item_handler.go) | `/presigned-url` 엔드포인트 추가             |
| [server/db/queries/media_items.sql](../server/db/queries/media_items.sql)                           | Presigned URL 관련 쿼리 추가                 |
| [ai/src/service/face.py](../ai/src/service/face.py)                                                 | 변경 없음 (재사용)                           |

### 신규 파일

| 파일                                                    | 내용                                         |
| ------------------------------------------------------- | -------------------------------------------- |
| `server/cmd/resize-lambda/main.go`                      | Resize Lambda 엔트리포인트                   |
| `server/Dockerfile.resize-lambda`                       | libvips 포함 Docker 이미지                   |
| `server/db/migrations/000002_add_async_pipeline.up.sql` | face_recognition_jobs 테이블 + 인덱스        |
| `server/db/queries/face_recognition_jobs.sql`           | sqlc 쿼리                                    |
| `ai/src/batch/face_recognition_batch.py`                | ECS AI 배치 스크립트 (SQS 없이 DB 폴링)      |
| `infra/s3.tf`                                           | S3 버킷, 이벤트 알림, CORS, Lifecycle        |
| `infra/sqs.tf`                                          | face-recognition-queue, resize-failed-dlq    |
| `infra/ecs.tf`                                          | ECS Cluster, Task Definition (Phase 1: Spot) |
| `infra/lambda_trigger_ecs.tf`                           | trigger-ecs Lambda + SQS Event Source        |
