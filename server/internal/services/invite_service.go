...
func (s *InviteService) CreateInviteToken(ctx context.Context, familyID string, groupID int32, createdByUserID string, familyTitle, customFamilyTitle string) (db.InviteToken, error) {
...
// ValidateInviteToken 토큰이 유효하면 초대 정보(그룹) 반환. 로그인 전 초대 링크 유효성 확인용.
func (s *InviteService) ValidateInviteToken(ctx context.Context, token string) (groupName string, familyID string, groupID int32, err error) {
	invite, err := s.inviteTokenStore.GetInviteTokenByToken(ctx, token)
	if err != nil || invite.ID == 0 {
		return "", "", 0, apperr.NewAppErrorWithData(apperr.NotFound, "error.invite_token_invalid", nil)
	}
	if _, err := s.familyStore.FindFamilyByID(ctx, invite.FamilyID); err != nil {
		return "", "", 0, apperr.NewAppErrorWithData(apperr.NotFound, "error.not_found", map[string]string{"field": "field.family"})
	}
	group, err := s.groupStore.FindGroupByID(ctx, invite.GroupID)
	if err != nil || group.ID == 0 {
		return "", "", 0, apperr.NewAppErrorWithData(apperr.NotFound, "error.not_found", map[string]string{"field": "field.group"})
	}
	return group.Name, invite.FamilyID, invite.GroupID, nil
}

// GetValidInviteToken 유효한 초대 토큰 조회 (OAuth/가입 시 사용할 family_id, group_id, family_title 획득용)
func (s *InviteService) GetValidInviteToken(ctx context.Context, token string) (familyID string, groupID int32, familyTitle, customFamilyTitle string, err error) {
	invite, err := s.inviteTokenStore.GetInviteTokenByToken(ctx, token)
	if err != nil || invite.ID == 0 {
		return "", 0, "", "", apperr.NewAppErrorWithData(apperr.NotFound, "error.invite_token_invalid", nil)
	}
...
func (s *InviteService) validateFamilyExists(ctx context.Context, familyID string) error {
